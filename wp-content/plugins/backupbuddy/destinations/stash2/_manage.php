...
if ( 'true' != pb_backupbuddy::_GET( 'listAll' ) ) {
	$listAll = '<a href="' . $url_prefix . '&listAll=true" style="text-decoration: none;">List all site\'s files</a>';
} else {
	$listAll = '<a href="' . $url_prefix . '&listAll=false" style="text-decoration: none;">Only list this site\'s files</a>';
}
?>
...
$files = pb_backupbuddy_destination_stash2::listFiles( $settings, $remotePath, $site_only );
if ( ! is_array( $files ) ) {
...
if ( count( $backup_list ) == 0 ) {
	echo '<br><b>';
	_e( 'Your remote BackupBuddy Stash storage does not contain any backup or Snapshot zip files yet.', 'it-l10n-backupbuddy' );
	echo '</b></center>';
} else {
	$tableArgs = array(
		'action'		=>	$url_prefix,
