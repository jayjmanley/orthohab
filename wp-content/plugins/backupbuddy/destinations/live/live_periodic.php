...
class backupbuddy_live_periodic {
	...
	*/
	...
	public static function run_periodic_process( $preferredStep = '', $preferredStepArgs = array() ) {
		...
		pb_backupbuddy::status( 'details', 'Live periodic process starting. Redirecting status logging temporarily.' );
	...
	private static function _step_send_pending_db_snapshots( $startAt = 0 ) {
		...
		if ( 0 == $startAt ) {
			...
			backupbuddy_core::render_dat_contents( $dat_settings, $dat_file );
			...
			if ( false === pb_backupbuddy_destinations::send( $destination_settings, $dat_file, $send_id, $delete_after = true, $isRetry = false, $trigger = 'live_periodic', $destination_id = backupbuddy_live::getLiveID() ) ) {
				pb_backupbuddy::status( 'error', 'Error #389398: Unable to send DAT file to Live servers. See error log above for details.' );
			}
		...
		foreach( self::$_tables as $table => &$tableDetails ) {
			...
			if ( $sendSizeSum > 0 ) {
				...
				$time_remaining = backupbuddy_core::adjustedMaxExecutionTime() - ( $time_elapsed + self::TIME_WIGGLE_ROOM ); // Estimated time remaining before PHP times out. Unit: seconds.
				...
				pb_backupbuddy::status ('details', 'Not the first DB file to send this pass. Send speed: `' . $send_speed . '` MB/sec. Time elapsed: `' . $time_elapsed . '` sec. Time remaining: `' . $time_remaining . '` sec based on reported max time of `' . backupbuddy_core::adjustedMaxExecutionTime() . '` sec with wiggle room `' . self::TIME_WIGGLE_ROOM . '` sec. Size possible with remaining time: `' . $size_possible_with_remaining_time . '` MB. Size to chunk (greater of filesize or chunk): `' . $size_to_send . '` MB. Conclusion: `' . $send_speed_status . '`.' );
	...
	private static function _step_process_table_deletions( $startAt = 0 ) {
		...
		foreach( self::$_tables as $table => &$tableDetails ) {
			...
			if ( ( microtime( true ) - $start_time + self::TIME_WIGGLE_ROOM )  > backupbuddy_core::adjustedMaxExecutionTime() ) { // Running out of time! Chunk.
				self::$_tablesObj->save();
	...
	private static function _step_update_files_list( $custom_root = '', $startAt = 0, $items = array() ) {
		...
		$excludes = array_unique( array_merge(
				backupbuddy_core::get_directory_exclusions( pb_backupbuddy::$options['profiles'][0], $trim_suffix = true, $serial = '' ),
				backupbuddy_core::get_directory_exclusions( array( 'excludes' => $excludes ), $trim_suffix = true, $serial = '' )
			) );
		...
		$max_time = backupbuddy_core::adjustedMaxExecutionTime() - self::TIME_WIGGLE_ROOM;
	...
	private static function _step_process_file_deletions( $startAt = 0 ) {
		...
		foreach( self::$_catalog as $signatureFile => &$signatureDetails ) {
			...
			if ( ( microtime( true ) - $start_time + self::TIME_WIGGLE_ROOM )  > backupbuddy_core::adjustedMaxExecutionTime() ) { // Running out of time! Chunk.
				...
				return array( 'Processing deletions', array( $loopCount-$filesDeleted ) );
			}
	...
	private static function _step_send_pending_files( $startAt = 0 ) {
		...
		foreach( self::$_catalog as $signatureFile => &$signatureDetails ) {
			...
			// If too many remote sends have failed today then give up for now since something is likely wrong.
			if ( self::$_state['stats']['recent_send_fails'] > $destination_settings['max_daily_failures'] ) {
				$error = 'Error #8439734: Too many file transfer failures have occurred. Halting sends for today.';
				backupbuddy_core::addNotification( 'live_error', 'BackupBuddy Stash Live Error', $error );
				self::$_state['step']['last_status'] =  $error;
				pb_backupbuddy::status( 'error', $error );
				return false;
			...
			if ( $sendSizeSum > 0 ) {
				...
				pb_backupbuddy::status ('details', 'Not the first normal file to send this pass. Send speed: `' . $send_speed . '` MB/sec. Time elapsed: `' . $time_elapsed . '` sec. Time remaining: `' . $time_remaining . '` sec based on reported max time of `' . backupbuddy_core::adjustedMaxExecutionTime() . '` sec. Size possible with remaining time: `' . $size_possible_with_remaining_time . '` MB. Size to chunk (greater of filesize or chunk): `' . $size_to_send . '` MB. Conclusion: `' . $send_speed_status . '`.' );
			} // end subsequent send time check.
		...
		pb_backupbuddy::status( 'details', 'Checked `' . $checkCount . '` items for sending. Sent `' . $sendAttemptCount . '`.' );
				...
				if ( ! isset( self::$_catalog[ '/' . $file['Key'] ] ) ) { // Remotely stored file not found in local catalog. Delete remote.
					...
					if ( count( $pendingDelete ) >= $deleteBatchSize ) {
						if ( true !== ( $delete_result = pb_backupbuddy_destination_live::deleteFile( $destination_settings, $pendingDelete ) ) ) {
							pb_backupbuddy::status( 'error', 'Error #4397347934: Unable to delete one or more remote files. See log above for details. Details: `' . $deleteResult . '`. Clearing pendingDelete var for next batch.' );
						} else {
							pb_backupbuddy::status( 'details', 'Deleted batch of `' . count( $pendingDelete ) . '` remote files. Clearning pendingDelete var for next batch.' );
	...
	public static function get_destination_settings() {
		...
		require_once( pb_backupbuddy::plugin_path() . '/destinations/live/init.php' );
		return pb_backupbuddy_destination_live::_formatSettings( pb_backupbuddy::$options['remote_destinations'][ backupbuddy_live::getLiveID() ] );
	...
	private static function _load_state( $force_load = false, $get_contents_only = false ) {
		...
		if ( true === $get_contents_only ) {
			...
			$read_only = false;
			$ignore_lock = self::_fileoptions_lock_ignore_timeout_value() + 999999;
		...
		if ( true !== ( $result = $stateObj->is_ok() ) ) {
			pb_backupbuddy::status( 'error', 'Error #3297392. Fatal error. Unable to create or access fileoptions file. Details: `' . $result . '`.' );
			die();
		}
	...
	private static function _load_catalog( $force_reload = false, $get_contents_only = false ) {
		...
		if ( true !== ( $result = $catalogObj->is_ok() ) ) {
			pb_backupbuddy::status( 'error', 'Error #239239034. Unable to create or access fileoptions file. Details: `' . $result . '`.' );
			return false;
		}
	...
	private static function _load_tables( $force_reload = false, $get_contents_only = false ) {
		...
		if ( true !== ( $result = $tablesObj->is_ok() ) ) {
			pb_backupbuddy::status( 'error', 'Error #435554390. Unable to create or access fileoptions file. Details: `' . $result . '`.' );
			return false;
		}
