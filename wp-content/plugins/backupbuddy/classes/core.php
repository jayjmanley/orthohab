...
class backupbuddy_core {
	 ...
	 */
	public static function schedule_single_event( $time, $method, $args, $rescheduleCount = 0, $limit_single = false ) {
		$tag = 'backupbuddy_cron';
		
		// TODO: Untested but should probably work:
		// Limit to only one instance of this tag scheduled. Useful for scheduled items which may keep trying to schedule over and over but not run due to cron issues such as W3TC.
		/*
		if ( true === $limit_single ) {
			...
			if ( FALSE !== $single_scheduled ) {
				pb_backupbuddy::status( 'warning', 'Warning #839249743: An un-run schedule appears to already exist for this tag which is limited to a single instance. This may be a temporary issue or this may be caused by cron interference such as by W3TC. Details: `' . print_r( $single_scheduled, true ) . '`.' );
				return false;
			}
		}
		*/
	...
	public static function unschedule_event( $time, $tag, $args ) {
		...
		if ( FALSE === $unschedule_result ) {
			pb_backupbuddy::status( 'warning', 'Warning: Unable to remove schedule as wp_unschedule_event() returned false. A plugin may have prevented it or it is already unscheduled. This may not be a problem.' );
			return false;
		}
		if ( FALSE !== $next_scheduled ) {
	...
	public static function render_dat_contents( $settings, $dat_file ) {
		...
		fwrite( $file_handle, $encoded_dat_content );
		fclose( $file_handle );
	...
	public static function php_runtime_test( $schedule_results = false ) {
		$total_loops = backupbuddy_constants::PHP_RUNTIME_TEST_MAX_TIME;
		...
		while( $loop_count <= $total_loops ) {
			...
			if ( false === fwrite( $fso, $time_elapsed ) ) {
				$total_loops = 0; // Stop since write failed.
			}
			$loop_start = microtime( true );
			while( ( microtime( true ) - $loop_start ) < 1 ) { // Can't sleep() because sleep doesn't count towards execution time.
	...
	public static function php_runtime_test_results() {
		...
		// Sanitize and verify numeric value.
