		...
		if ( ( false !== $ignore_lock ) && ( true === $this->is_locked() ) ) {
			...
			if ( false === $ignore_lock ) {
				...
				if ( true === $this->is_locked() ) { // Check lock one more time.
					sleep( 3 ); // Wait 1 second befoe trying again.
					if ( true === $this->is_locked() ) { // Check lock one more time.
			...
			if ( is_numeric( $ignore_lock ) ) {
				if ( false === ( $modified = @filemtime( $this->_file ) ) ) {
					pb_backupbuddy::status( 'warning', 'Warning #5349847. Unable to read fileoptions file MTIME of locked file `' . $this->_file . '`. Lock file ID: ' . $this->_last_seen_lock_id . '.' );
					$this->_is_ok = 'ERROR_CANT_READ_MTIME';
					return false;
				}
				if ( ( time() - $modified ) < $ignore_lock ) {
					pb_backupbuddy::status( 'warning', 'Warning #54556. Unable to read fileoptions file `' . $this->_file . '` as it is currently locked AND not enough time has passed (time passed: `' . ( time() - $modified ) . '`, limit: `' . $ignore_lock . '`). Lock file ID: ' . $this->_last_seen_lock_id . '.' );
					$this->_is_ok = 'ERROR_LOCKED';
					return false;
				}
				pb_backupbuddy::status( 'warning', 'Fileoptions file was locked `' . ( time() - $modified ) . '` seconds ago BUT the lock time of `' . $ignore_lock . '` seconds was exceeded.' );
			}
