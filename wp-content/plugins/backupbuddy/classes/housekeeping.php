...
class backupbuddy_housekeeping {
	...
	public static function schedule_php_runtime_tests( $force_run = false ) {
		...
		if ( pb_backupbuddy::$options['last_tested_php_runtime'] > 0 ) { // if it's run at least once...
			...
			if ( $elapsed < backupbuddy_constants::PHP_RUNTIME_TEST_INTERVAL ) { // Not enough time elapsed since last run.
				pb_backupbuddy::status( 'details', 'Not enough time elapsed since last PHP runtime test interval. Waiting until next housekeeping (or longer). Elapsed: `' . $elapsed . '`. Interval limit: `' . backupbuddy_constants::PHP_RUNTIME_TEST_INTERVAL . '`.' );
				return;
			}
		}
