			...
			*/
			...
			if ( 0 != $max_time ) { // Chunking enabled.
				if ( ( time() - $start_time ) > $max_time ) { // Not enough time left.
					pb_backupbuddy::status( 'details', 'Running out of time calculating deep file scan. Chunking at position `' . $startAt . '`. Elapsed: `' . ( time() - $start_time ) . '` secs. Max time: `' . $max_time . '` secs.' );
